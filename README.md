...
```bash
npm install
npm run dev       # http://localhost:4321/CryptoTaxSoftwareReview/
...
This repo is prepared for SEO coverage around crypto tax software. Starter articles live under `src/content/reviews/` and use review/comparison schema, RSS, JSON feed, sitemap, and base-path-safe internal links.
...
Push this project to a GitHub repository named `CryptoTaxSoftwareReview` and enable GitHub Pages from Actions. The workflow at `.github/workflows/deploy.yml` builds and publishes `dist/` automatically.
